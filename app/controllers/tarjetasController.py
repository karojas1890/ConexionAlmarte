...
def AddCard():
    try:
        ...
        numeroHash = bcrypt.hashpw(numeroTarjeta.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        fechaHash=bcrypt.hashpw(fechaExpiracion.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        nueva_tarjeta = Tarjeta(
            id_usuario=idusuario,  
            nombre_titular=data.get('cardHolder'),
            ultimo4=ultimos4,
            numero_tarjeta=numeroHash,
            fecha_expiracion=fechaHash,
            estado=True
        )
